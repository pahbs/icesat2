...
lm_eqn = function(df){
      ...
      return(data.frame(cbind(int, slope, r2, rmse, sig2, n)))
}
lm_mod = function(df){
      y=df[,1]
      x=df[,2]
...
do_cover_plots = function(df, p_title, y_var, x_var){
  ...
  val_plot_geom_list = list(
          ...
          scale_x_continuous(breaks = seq(0,ymax,0.1), limits = c(0,ymax)),
          scale_y_continuous(breaks = seq(0,ymax,0.1), limits = c(0,ymax)),
          scale_fill_gradientn( labels = leg_breaks, breaks =leg_breaks, limits = leg_lims, colours=pal_rev_spectral9, name='# obs.'),
          theme_minimal()+theme(axis.text = element_text(size = 6)),
          coord_fixed()
          )
  
  # Assign names to categorical variables that can be used to subset data
  df$cloud_flg = plyr::mapvalues(factor(df$cloud_flg), from = c(0,1,2,3,4,5), to=c("High conf. clear","Med conf. clear", "Low conf. clear", "Low conf. cloudy ", "Med conf. cloudy", "High conf. cloudy"))
  df$seg_snow  = plyr::mapvalues(factor(df$seg_snow), from = c(0,1,2,3), to=c("ice free water", "snow free land","snow", "ice"))
  df$night_flg = plyr::mapvalues(factor(df$night_flg), from = c(0,1), to=c("day","night"))
  df$tcc_flg   = plyr::mapvalues(factor(df$tcc_flg), from = c(0,1), to=c("≤ 5%","> 5%"))
  
  # Create the equation labels for the two groups
  source(paste("C:\\Users\\pmontesa\\Google Drive\\Work\\R","\\","plot_functions.R",sep=''))
  
  # Single plot: night_flg
  df_eq <- ddply(df %>% dplyr::select_(y_var, x_var, "cloud_flg", "night_flg", "tcc_flg", "gt" ),.(night_flg),c("lm_eqn") )
  df_eq$slope = as.numeric(as.character(df_eq$slope))
  
  p_single = ggplot(df, aes_string(y=y_var, x=x_var)) + 
    labs(title = p_title, subtitle = paste0("Reference height: ",x_var_to)) + 
    geom_bin2d(binwidth = c(0.025,0.025))+
    #geom_point(alpha=0.25)+
    labs(y=paste0("Cover (fraction); ATL08, ",y_var), x=paste0("Cover (fraction); ", x_var))+
    val_plot_geom_list+
    geom_text(data=df_eq, aes(x = Inf, y = 0,label=paste("r^2 ==", r2,sep='')),    inherit.aes=FALSE, parse = TRUE, hjust="inward", vjust="inward", size=sz_txt)+
    geom_text(data=df_eq, aes(x = Inf, y = 0+3,label=paste0("rmse ==", rmse)),     inherit.aes=FALSE, parse = TRUE, hjust="inward", vjust="inward", size=sz_txt)+
    geom_text(data=df_eq, aes(x = Inf, y = 0+3*2,label=paste0("bias ==", int)),    inherit.aes=FALSE, parse = TRUE, hjust="inward", vjust="inward", size=sz_txt)+
    geom_text(data=df_eq, aes(x = Inf, y = 0+3*3,label=paste0("slope ==", slope)), inherit.aes=FALSE, parse = TRUE, hjust="inward", vjust="inward", size=sz_txt)+
    geom_text(data=df_eq, aes(x = Inf, y = 0+3*4,label=paste0("n ==", n)),         inherit.aes=FALSE, parse = TRUE, hjust="inward", vjust="inward", size=sz_txt)+
...
do_height_plots = function(df, ref_name, filt_type, filt_str, y_var, x_var, outdir){
  ...
  # Single plot: night_flg
  df_mods <- dlply(df %>% dplyr::select_(y_var, x_var, "msw_flg","cloud_flg", "night_flg", "tcc_flg", "gt" ), .( night_flg, msw_flg), c("lm_mod"))
  df_eq   <- ddply(df %>% dplyr::select_(y_var, x_var, "msw_flg","cloud_flg", "night_flg", "tcc_flg", "gt" ), .( night_flg, msw_flg), c("lm_eqn") )
  df_eq$slope = round(as.numeric(as.character(df_eq$slope)), dig=2)
  
  plot_ht_val_MSW = ggplot(df, aes_string(y=y_var, x=x_var)) + 
    ...
    theme(axis.text = element_text(size = 10), strip.text = element_text(size = 12))
  ggsave(plot = plot_ht_val_MSW,
         file = path(outdir,outplotdir, paste0(paste("plot_ht_val_MSW",ref_name,filt_type,y_var,x_var,sep="_"),".png")),
         device = 'png', dpi = 300, width = 12, height = 6)
  
  
  # Facet plot night_flg~gt
  df_eq <- ddply(df %>% dplyr::select_(y_var, x_var, "cloud_flg", "night_flg", "tcc_flg", "gt","ref_tcc_bin" ),.(ref_tcc_bin, night_flg),c("lm_eqn") )
  df_eq$slope = round(as.numeric(as.character(df_eq$slope)), dig=2)
  ...
  plot_ht_val_TCC = ggplot(df, aes_string(y=y_var, x=x_var)) + 
    ...
    labs(y=paste0("Height (m); ATL08, ",y_var), x=paste0("Height (m); ", x_var))+
    val_plot_geom_list+
    geom_text(data=df_eq, aes(x = Inf, y = 0,     label=paste0("r^2 ==", r2)),     inherit.aes=FALSE, parse = TRUE, hjust="inward", vjust="inward", size=sz_txt)+
    geom_text(data=df_eq, aes(x = Inf, y = 0+3,   label=paste0("rmse ==", rmse)),  inherit.aes=FALSE, parse = TRUE, hjust="inward", vjust="inward", size=sz_txt)+
    geom_text(data=df_eq, aes(x = Inf, y = 0+3*2, label=paste0("bias ==", int)),   inherit.aes=FALSE, parse = TRUE, hjust="inward", vjust="inward", size=sz_txt)+
    geom_text(data=df_eq, aes(x = Inf, y = 0+3*3, label=paste0("slope ==", slope)),inherit.aes=FALSE, parse = TRUE, hjust="inward", vjust="inward", size=sz_txt)+
    geom_text(data=df_eq, aes(x = Inf, y = 0+3*4, label=paste0("n ==", n)),        inherit.aes=FALSE, parse = TRUE, hjust="inward", vjust="inward", size=sz_txt)+
    facet_grid(night_flg~ref_tcc_bin)+
    theme(axis.text = element_text(size = 10), strip.text = element_text(size = 12))
  ...
  return(list(plot_ht_val_MSW,plot_ht_val_TCC))
}
...
if(!exists("df_atl08")){
  df_atl08 = read.csv(path(atl08_dir, "v3", "ATL08_merged_003_01_filt_45_90_-180_-13.csv"))
}
dim(df_atl08)

# Read in Zonal Stats files
dir_zonal = "D:\\projects\\3dsi\\ZonalStats\\"
df_atl08_gliht_in = read.csv(path(dir_zonal, "ATL08__GLIHT__ZonalStats.csv"))
df_atl08_lvis_in  = read.csv(path(dir_zonal, "ATL08__LVIS__ZonalStats.csv"))
...
# L17: <stackName>_LS7_CC_warp.tif
# L18: C2C_change_year_warp.tif
# L19: C2C_change_type_warp.tif
# L20: MCD12Q1_A2017001_LC_Type1_warp.tif
# L21: boreal_clust_25_100_2019_10_8_warp.tif
# L22: PCA_NaN_1_093019_warp.tif
# L23: PCA_NaN_2_093019_warp.tif
# L24: PCA_NaN_3_093019_warp.tif
# L25: wc2.0_bio_30s_01_warp.tif
# L26: wc2.0_bio_30s_10_warp.tif
# L27: wc2.0_bio_30s_18_warp.tif
# L28: UiO_PEX_PERPROB_3.0_20171201_2000_2016_warp.tif
# L29: BDTICM_M_250m_warp.tif
...
# Set col names
names(df_atl08_lvis_in)[grep(".median", names(df_atl08_lvis_in))][1:11] = c(paste0("rh", c(seq(10,90,10), 95), ".median"), "tcc1.37.median")
...
# Join new fields from filtered ATL08 not available in Maggie's Zonal db
df_atl08_LIDAR = left_join(df_atl08_LIDAR, 
                           df_atl08 %>% select(contains('rh'), lat, lon, asr, h_dif_ref, ter_flg, ph_rem_flg, dem_rem_flg, seg_wmask, lyr_flg), by = c("lat", "lon"))
...
df_atl08_LIDAR$seg_snow  = mapvalues(factor(df_atl08_LIDAR$seg_snow), from = c(0,1,2,3),     to=c("ice free water", "snow free land","snow", "ice"))
...
print("Number of airborne lidar validation points")
print(dim(df_atl08_LIDAR)[1])
df_atl08_LIDAR %>% 
      dplyr::group_by(lidar_name, night_flg, m) %>% 
      dplyr::summarise(n_obs = n())

# Stacked histograms
ggplot(df_atl08_LIDAR, aes(x=ref_ht, fill=as.factor(msw_flg)))+geom_histogram( position = "stack", binwidth = 2) + facet_grid(night_flg~m) + scale_fill_brewer(palette = "Set1") + theme_minimal()
```
...
print(do_height_plots(df_atl08_LIDAR %>% drop_na(ref_tcc_bin, ref_ht_bin), ref_name, filt_type, filt_str, "h_can", "ref_ht", dir_zonal))
...
print(do_height_plots(df_atl08_LIDAR_FILT %>% drop_na(ref_tcc_bin, ref_ht_bin), ref_name, filt_type, filt_str, "h_can", "ref_ht", dir_zonal))
